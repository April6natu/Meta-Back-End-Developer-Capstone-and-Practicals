...
def calculate_subtotal(order):
    ...
    subtotal = sum(item["price"] for item in order)   # ✅ add up prices
...
def calculate_tax(subtotal):
    ...
    return round(tax, 2)       # ✅ round to 2 decimal places


def summarize_order(order):
    """Summarizes the order."""
    print_order(order)  
    subtotal = calculate_subtotal(order)   # ✅ get subtotal
    tax = calculate_tax(subtotal)          # ✅ get tax
    total = round(subtotal + tax, 2)       # ✅ final total
    names = [item["name"] for item in order]  # ✅ extract item names
    return names, total
